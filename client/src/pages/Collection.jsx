            ...
            <div className='grid grid-col-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 gap-y-6'>
                {
                    filterProducts.map((item, index) => (
                        ...
                        <div key={index} id={item._id} className="LatestProduct bg-slate-500 w-full h-80  md:h-60  lg:h-80 mt-3 relative rounded-lg overflow-hidden">
                            <div className="imgbox h-3/5" >
                                <img src={item.image} alt={item.name} className="w-full h-full object-cover rounded-t-lg" />
                            </div>

                            <div className="Latestdetails px-4 py-2 lg:px-3 lg:py-3 bg-gray-700 bottom-[-130px] lg:bottom-[-195px] absolute bg-opacity-90">
                            <div className='flex justify-between'>
                                <div className='w-24 md:w-24 lg:w-36 '>
                                <h2 className="text-custom-xs lg:text-sm font-semibold">{item.name}</h2>
                                </div>
                                <div className="Latestprice right-[10px] lg:right-[20px] text-custom-Sx lg:text-lg mt-2">$ {item.price}</div>
                            </div>
